...
### 1. Environment setup (add a streamlined version of Colin's article here)
        **A.** .zshrc modifications (handles brew and nvm issues).
        **B.** Starship config xarch (architecture switching function in .toml).
        **C.** VSCode setup.

### 2. Project setup - provide instructions on `direnv` virtual environment setup for any given project and create a link in `Jade Truffle` boilerplate documentation as reference for M1 users.
